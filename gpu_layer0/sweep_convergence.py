"""sweep_convergence.py — convergence-time-vs-deviation sweep.

Pure oscillator math, no GPU/pynvml dependency -- deviation is driven
directly rather than sampled from telemetry, so this runs anywhere,
not just Kaggle. Answers: how many report intervals does it take to
lock (r >= LOCK_THRESHOLD, sustained) starting from a given constant
deviation, and does near-zero deviation correctly NOT lock (gain->0
should mean the phases free-run, not artificially converge)?
"""
from layer0_oscillator import (Layer0Node, gain_from_deviation,
                                integrate_interval, order_parameter)

N_NODES = 2                # matches the real Kaggle run (2x T4)
INTERVAL_S = 0.5
MAX_INTERVALS = 40         # 20s ceiling per deviation level
LOCK_THRESHOLD = 0.99
LOCK_SUSTAIN = 3           # must stay >= threshold for this many consecutive intervals

DEVIATIONS = [0.0, 0.02, 0.05, 0.10, 0.20, 0.30, 0.50, 0.80]


def run_one(deviation, seed_base=0):
    nodes = [Layer0Node(i + seed_base) for i in range(N_NODES)]
    gain = gain_from_deviation(0.70 + deviation, 0.70)  # constant deviation each interval
    r_trace = []
    locked_at = None
    sustain_count = 0
