/**
 * quartz_cellstate_node_b.c — second independent walker over the same
 * melanoma cellstate landscape as quartz_cellstate_node.c, started from
 * the opposite corner of the embedding instead of the grid center.
 * Purpose: if both walkers converge to the same low-energy region, the
 * landscape has real structure; if they land in different basins, that's
 * evidence of distinct attractors (candidate subpopulations).
 *
...
#define STATE_PATH "/tmp/cellstate_state_b.json"
...
// see quartz_cellstate_node.c for why this exists: without it, energy goes
// flat outside the grid and the walker random-walks off the data manifold.
#define K_WALL 0.05
...
int main(int argc, char *argv[]) {
    ...
    printf("=== QUARTZ CELLSTATE NODE B (opposite-corner start) ===\n");
    ...
    // start at the opposite corner of the bounding box from node A's center start
    double pos[3] = {
        L.ax[L.nx - 1], L.ay[0], L.az[L.nz - 1]
    };
