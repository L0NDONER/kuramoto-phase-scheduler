#define _GNU_SOURCE
/*
 * pi2_dvfs_reader.c — temlum controller for Pi2 DVFS plant.
 *
 * Neuron is identical to pi2_reader.c. Plant is different:
 * intent goes to pi2_dvfs_actuator on :7433 which steps cpufreq.
 *
 * Inputs:
...
/* ── control params (identical to cpuset neuron) ─────────────────────────── */
...
#define DCN_DVFS_PORT 7432  /* own DCN port — :7430 is owned by cpuset neuron */
#define INTENT_PORT   7433  /* DVFS actuator — distinct from cpuset :7431 */
...
int main(int argc, char **argv) {
    ...
    /* DCN receive socket — own port so cpuset neuron doesn't steal packets */
    ...
    printf("[pi2_dvfs_reader] T_target=%.1f P_target=%.4f dcn=:%d intent=%s:%d\n",
           T_TARGET, P_TARGET, DCN_DVFS_PORT, intent_ip, INTENT_PORT);
    fflush(stdout);
