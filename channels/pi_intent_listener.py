...
sys.path.insert(0, "/home/martin/claude/quartz_core")
from i0_phase_auth import gate_check
import socket
...
INTENTS = {
    ...
    "restart_container": ["docker", "restart", "portainer_agent"],
}
...
# Verb classes gated by both truthd quorum AND live phase-auth presence.
# BENIGN_READ skips phase_auth -- gate_check() is a multicast round trip
# (can take several seconds), not worth the latency for read-only intents.
# This is a SECOND gate alongside the HMAC signature, not a replacement:
# the key still proves sender identity + blocks replay (see
# pi_intent_common.py); phase_auth only proves the LAN substrate is
# currently alive and observable, which the static key can never prove
# no matter how well it's guarded.
PHASE_AUTH_VERB_CLASSES = {"LOCAL_HEAL", "LOCAL_DESTRUCTIVE"}

# 2026-09-01: phase_auth used to run inline per-command (~2s multicast
# round trip on every LOCAL_HEAL/DESTRUCTIVE call). Replaced with a
# background rotation: a daemon thread calls gate_check() on a fixed
# cadence and stamps the wall-clock time of the last PASS; the listener
# just checks whether that stamp is still fresh. Trades per-command
# freshness for latency -- there's up to ROTATION_INTERVAL_S of exposure
# if the substrate dies right after a successful rotation, accepted per
# user 2026-09-01 given ROTATION_INTERVAL_S <= STALENESS_TTL_S. UTC
# wall-clock, not tick count, so a fully stalled/dead beacon (zero new
# ticks forever) still correctly goes stale instead of looking frozen-fresh.
ROTATION_INTERVAL_S = 3
STALENESS_TTL_S      = 5

_phase_auth_last_pass = 0.0   # 0.0 = never passed -> correctly reads as stale
_phase_auth_lock = threading.Lock()


def _phase_auth_rotation_loop():
    # 2026-09-01: was calling gate_check() (verbose) and logging every
    # single cycle -- at ROTATION_INTERVAL_S=3s that's ~144k journal
    # lines/day (5 lines from phase_auth's own internal prints + 1 here),
    # almost entirely PASS-after-PASS noise. gate_check(verbose=False)
    # silences phase_auth's internal prints; logging here now only fires
    # on a state TRANSITION (first pass, or flipping PASS<->FAIL), not on
    # every steady-state confirmation -- a stuck-FAIL state still shows
    # up once when it starts, not every 3s forever.
    global _phase_auth_last_pass
    last_logged_state = None   # None = not yet logged, then True/False
    while True:
        passed = gate_check(verbose=False)
        if passed:
            with _phase_auth_lock:
                _phase_auth_last_pass = time.time()
            if last_logged_state is not True:
                print(f"[phase_auth_rotate] PASS, stamped {_phase_auth_last_pass:.1f}", flush=True)
                last_logged_state = True
        else:
            if last_logged_state is not False:
                print("[phase_auth_rotate] FAIL, not stamping (last pass stays as-is)", flush=True)
                last_logged_state = False
        time.sleep(ROTATION_INTERVAL_S)


def _phase_auth_is_fresh() -> bool:
    with _phase_auth_lock:
        last = _phase_auth_last_pass
    return (time.time() - last) <= STALENESS_TTL_S
...
def _run_at(intent, trigger_at, cmd, nonce, addr, sock, key, verb_class):
    ...
    if verb_class in PHASE_AUTH_VERB_CLASSES:
        if not _phase_auth_is_fresh():
            age = time.time() - _phase_auth_last_pass
            print(f"[listener] PHASE_AUTH DENIED intent={intent!r} -- last pass {age:.1f}s ago "
                  f"(TTL {STALENESS_TTL_S}s), not executed", flush=True)
            try:
                sock.sendto(pack_response(nonce, REJECTED, "gate: phase_auth stale (no live prover)", key), addr)
            except OSError:
                pass
            return
...
def main():
    ...
    threading.Thread(target=_phase_auth_rotation_loop, daemon=True).start()
    print(f"[listener] bound {BIND_IP}:{BIND_PORT}, intents={list(INTENTS)}, "
          f"phase_auth rotation every {ROTATION_INTERVAL_S}s (TTL {STALENESS_TTL_S}s)", flush=True)
